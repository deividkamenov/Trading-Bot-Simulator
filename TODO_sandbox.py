...
class MovingAverage(base_class.BaseClass):
    ...
    #     cumsum = self.data['Adj Close'].rolling(window=ma_1 + ma_2, min_periods=ma_2).apply(lambda x: x[-ma_2:].sum(),
    #                                                                                         raw=True)
    #
    #     # Divide the cumsum by the window size to get the moving average
    #     self.data[f'MA_{ma_1}'] = cumsum / ma_1
    #     self.data[f'MA_{ma_2}'] = cumsum / (ma_1 + ma_2)
    #
    #     # Use list comprehension instead of loop
    #     signals = [buy_signals.append(self.data['Adj Close'].iloc[i]) if self.data[f'MA_{ma_1}'].iloc[i] >
    #                                                                      self.data[f'MA_{ma_2}'].iloc[
    #                                                                          i] and trigger != 1 else
    #                sell_signals.append(self.data['Adj Close'].iloc[i]) if self.data[f'MA_{ma_1}'].iloc[i] <
    #                                                                       self.data[f'MA_{ma_2}'].iloc[
    #                                                                           i] and trigger != -1 else
    #                (buy_signals.append(float('nan')), sell_signals.append(float('nan')))
    #                for i in range(len(self.data))]
    #
    #     for i in range(len(signals)):
    #         if buy_signals[i] != float('nan'):
    #             my_stocks = invested_capital / buy_signals[i]
    #             trigger = 1
    #         elif sell_signals[i] != float('nan'):
    #             invested_capital = my_stocks * sell_signals[i]
    #             trigger = -1
    #
    #     if to_plot:
    #         self.plot_graph(buy_signals, sell_signals, ma_1, ma_2)
